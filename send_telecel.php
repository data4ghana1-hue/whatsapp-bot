...
// Calculate total available Telecel GB from helper
try {
    $availableGb = getNetworkBalance($pdo, $userId, 'Telecel');
} catch (Exception $e) {
    $availableGb = 0.0;
}
...
$isTelecelActive = !empty($settings['telecel_enabled']);

// Handle Form Submission
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($role !== 'admin' && !$isTelecelActive) {
        $errors[] = "Telecel Data service is currently offline for maintenance. Please check back shortly.";
    } else {
    ...
    if ($mode === 'bulk') {
        ...
        if (empty($bulkData)) {
            ...
            foreach ($lines as $line) {
                ...
                if (count($parts) >= 2) {
                    ...
                    $validPhone = normalizeAndValidatePhone($recip, 'Telecel');
                    if ($amt >= 2 && $amt <= 100 && $validPhone) {
                        $totalAmount += $amt;
            ...
            if (!empty($invalidLines)) {
                $errors[] = "Invalid entries (bad format, invalid Telecel number, or amount not between 2 and 100 GB): " . implode("; ", array_slice($invalidLines, 0, 3)) . (count($invalidLines) > 3 ? "..." : "");
                ...
                $mtnUp2GbApi = MtnUp2uPortalApi::isNetworkEnabled('Telecel') ? new MtnUp2uPortalApi() : null;
                $mtnUp2uApi  = MtnUp2uApi::isNetworkEnabled('Telecel')  ? new MtnUp2uApi()  : null;
                $nitghtApi   = NitghtApi::isNetworkEnabled('Telecel')   ? new NitghtApi()   : null;
                $s1Api = SupplierApi::isNetworkEnabled('Telecel') ? new SupplierApi() : null;
                $s2Api = Supplier2Api::isNetworkEnabled('Telecel') ? new Supplier2Api() : null;
                $s3Api = Supplier3Api::isNetworkEnabled('Telecel') ? new Supplier3Api() : null;
                $s4Api = Supplier5Api::isNetworkEnabled('Telecel') ? new Supplier5Api() : null;
                $backupApi = BackupApi::isNetworkEnabled('Telecel') ? new BackupApi() : null;
                $useSupplier =
                ...
                foreach ($validOrders as $order) {
                    ...
                    try {
                        $stmt = $pdo->prepare("INSERT INTO bundle_sends (user_id, network, recipient_phone, gb_amount, status) VALUES (?, 'Telecel', ?, ?, 'pending')");
                        $stmt->execute([$userId, $order['recipient'], $order['amount']]);
                        $orderId = (int)$pdo->lastInsertId();
                        
                        if ($useSupplier) {
                            $resolvedSupp = resolve_order_supplier('Telecel', $order['amount'], $order['recipient'], $pdo);
                            $supplierApi  = $resolvedSupp['api'];
                            $supplierName = $resolvedSupp['name'];
                            
                            if ($supplierApi) {
                                $apiResult = $supplierApi->sendBundle('Telecel', $order['recipient'], $order['amount'], $orderId);
                                
                                if ($apiResult['success']) {
                                    $status = Supplier5Api::mapStatus('Telecel', $apiResult);
                                    if ($supplierName === 'MTNUP2U PORTAL' || $supplierName === 'MTNUP2 GB') {
                                        $status = MtnUp2GbApi::mapStatus('Telecel', $apiResult);
                                    } elseif ($supplierName === 'MTNUP2U') {
                                        $status = MtnUp2uApi::mapStatus('Telecel', $apiResult);
                                    } elseif ($supplierName === 'NITGHT') {
                                        $status = NitghtApi::mapStatus('Telecel', $apiResult);
                                    } elseif ($supplierName === 'Supplier 1') {
                                        $status = SupplierApi::mapStatus('Telecel', $apiResult);
                                    } elseif ($supplierName === 'Supplier 2') {
                                        $status = Supplier2Api::mapStatus('Telecel', $apiResult);
                                        if ($status === 'processing') $status = 'unverified';
                                    } elseif ($supplierName === 'Supplier 3') {
                                        $status = Supplier3Api::mapStatus('Telecel', $apiResult);
                                        if ($status === 'processing') $status = 'waiting';
                                    } elseif ($supplierName === 'Backup') {
                                        $status = BackupApi::mapStatus('Telecel', $apiResult);
                                    }
        ...
        if ($amount < 2 || $amount > 100) {
            ...
            try {
                ...
                $resolvedSupp = resolve_order_supplier('Telecel', $amount, $recipient, $pdo);
                $supplierApi  = $resolvedSupp['api'];
                $supplierName = $resolvedSupp['name'];    
                    if ($supplierApi) {
                        $apiResult = $supplierApi->sendBundle('Telecel', $recipient, $amount, $orderId);
    ...
    if (isset($success) && $success !== '' && $availableGb < 5) {
        if (empty($_SESSION['low_balance_notified_Telecel'])) {
            ...
            if ($userPhone) {
                $smsApi = new SmsApi();
                $message = "Low Telecel balance alert! Your new balance is " . number_format($availableGb, 2) . " GB. Please top up soon.";
                $smsApi->sendSms($userPhone, $message);
                $_SESSION['low_balance_notified_Telecel'] = true;
            }
        }
    } elseif ($availableGb >= 5) {
        unset($_SESSION['low_balance_notified_Telecel']);
    }
}
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no, viewport-fit=cover">
<title>Send Telecel Bundle Â· Apex Prime Admin</title>
...
<style>
  ...
  .dashboard-page .stat-card-telecel { 
      background: linear-gradient(135deg, #fecaca 0%, #f87171 100%); 
      border: 1px solid #ef4444; 
      box-shadow: 0 4px 15px rgba(239, 68, 68, 0.2); 
      color: #7f1d1d;
  }
  .dark .dashboard-page .stat-card-telecel { 
  ...
  .mode-switch-container {
      --switch-color: #ef4444;
      --switch-color-shadow: rgba(239, 68, 68, 0.3);
  }
...
<div class="app-wrapper">
    ...
    <main class="main-content" style="padding: 0; background-color: #f8fafc;">
        ...
        <div class="container fade-up" style="max-width: 800px; margin: 0 auto; padding-top: 1.5rem;">
            ...
            <!-- Telecel Banner -->
            <div style="background: linear-gradient(135deg, #dc2626 0%, #ef4444 100%); color: white; padding: 1.25rem 1.5rem; text-align: center; margin-bottom: 1.5rem; box-shadow: 0 4px 15px rgba(239, 68, 68, 0.25); border-radius: 0 0 24px 24px;">
                <h1 style="font-size: 1.35rem; font-weight: 800; color: #ffffff; margin-bottom: 0.4rem; letter-spacing: 0.02em; text-transform: uppercase;"><i class="fas fa-broadcast-tower" style="margin-right: 8px;"></i> TELECEL DATA BUNDLE</h1>
                <p style="font-size: 0.9rem; font-weight: 500; color: #ffffff; opacity: 0.9; margin: 0 auto; line-height: 1.4; max-width: 600px;">Fast & reliable connectivity with Telecel high-speed bundles and strong network coverage.</p>
            </div>
            
            <div class="grid grid-cols-1" style="gap: 1.5rem; margin-bottom: 2rem;">
                <?php if (!$isTelecelActive && $role !== 'admin'): ?>
                    <div style="background: rgba(239, 68, 68, 0.1); border: 1.5px solid rgba(239, 68, 68, 0.3); border-radius: 16px; padding: 1.25rem; margin-bottom: 1.5rem; display: flex; align-items: center; gap: 1rem; color: #ef4444;">
                        ...
                        <div>
                            <div style="font-weight: 800; font-size: 0.95rem; margin-bottom: 0.2rem;">Telecel Service Maintenance</div>
                            <div style="font-size: 0.82rem; color: var(--text-secondary); line-height: 1.4;">
                                Telecel Data orders are temporarily offline for scheduled maintenance. Please check back shortly or try another network.
                            </div>
                        </div>
                    </div>
                <?php endif; ?>
                ...
                <!-- Send Form -->
                <div class="card" <?= (!$isTelecelActive && $role !== 'admin') ? 'style="opacity: 0.6; pointer-events: none;"' : '' ?>>
                        ...
                        <div style="max-width: 380px; margin: 0 auto; width: 100%;">
                            <div id="singleFields">
                                ...
                                <div style="margin-bottom: 1.5rem;">
                                    ...
                                    <input type="number" step="0.01" min="2" max="100" name="amount" id="amountInput" required class="form-control" placeholder="e.g. 5">
                                </div>
                            </div>
...
<div id="fullScreenLoader">
    <div style="background: white; padding: 2rem; border-radius: 1rem; display: flex; flex-direction: column; align-items: center; gap: 1rem;">
        <svg width="40" height="40" viewBox="0 0 24 24" fill="none" style="animation: spin 1s linear infinite;">
            <circle cx="12" cy="12" r="10" stroke="#fecaca" stroke-width="4"/>
            <path d="M12 2a10 10 0 0 1 10 10" stroke="#ef4444" stroke-width="4" stroke-linecap="round"/>
        </svg>
        <div style="color: #7f1d1d; font-weight: 700; font-family: Inter, sans-serif;">Sending Bundle...</div>
    </div>
</div>
...
function loadSampleBulk() {
    const sampleText = "0201234567 1.5\n0507654321 2.0";
...
document.getElementById('bulkInput').addEventListener('input', function() {
    ...
    lines.forEach(line => {
        ...
        if (parts.length >= 2) {
            ...
            let validPrefix = telPrefixes.includes(num.substring(0, 3));
            if (num.match(/^[0-9]{10}$/) && validPrefix && !isNaN(gb) && gb > 0) {
                ...
                if (!num.match(/^[0-9]{10}$/)) {
                    ...
                    errorReason = "Contains non-Telecel number";
                } else {
